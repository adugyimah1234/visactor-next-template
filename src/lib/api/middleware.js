...
const failedLoginAttemptsCache = new Map();
...
export function csrfProtection() {
    return async function csrfMiddleware(req) {
...
export function rateLimit(options) {
    ...
    return async function rateLimitMiddleware(req) {
        ...
        if (requestCount > maxRequests) {
            ...
            const resetTime = ((_a = rateLimitCache.get(key)) === null || _a === void 0 ? void 0 : _a.resetTime) || Date.now() + windowMs;
        ...
        if (includeHeaders) {
            ...
            const resetTime = (rateLimitInfo === null || rateLimitInfo === void 0 ? void 0 : rateLimitInfo.resetTime) || Date.now() + windowMs;
            response.headers.set('X-RateLimit-Limit', maxRequests.toString());
    ...
    return async function validationMiddleware(req) {
        try {
            ...
            if (!result.success) {
                ...
                return errorResponse(new ValidationError('Validation error', formatted), 422);
            }
            return { data: result.data, validation: { success: true } };
        }
        catch (error) {
            return errorResponse(new ValidationError('Invalid request body'), 400);
        }
    };
}
/**
...
export function logRequest(options = {}) {
    const { logLevel = 'info', includeHeaders = false } = options;
    return async function loggerMiddleware(req, _context = {}) {
 ...
 */
export function logRequestCompletion(req, response, context) {
    ...
    if (context.requestStartTime) {
        ...
        console.log(`[${new Date().toISOString()}] ${context.requestId || 'UNKNOWN'} - Completed ${req.method} ${req.url} - ${status} - ${duration}ms`);
    }
    return response;
}
/**
 ...
 */
export function trackFailedLoginAttempt(key) {
    const now = Date.now();
 ...
 */
export function calculateLoginDelay(attempts) {
...
export const withMiddleware = (handler) => {
    return async (req, context = {}) => {
        try {
            ...
            const enrichedContext = Object.assign(Object.assign({}, context), logContext);
