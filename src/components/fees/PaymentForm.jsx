...
import { createPayment, getPaymentMethods } from '@/services/payment-processing';
// Student service
...
export default function PaymentForm({ schoolId, onPaymentSuccess }) {
    // Student state
    ...
    const loadOutstandingFees = async (studentId) => {
        setLoadingFees(true);
        try {
    ...
    const handlePaymentMethodChange = (value) => {
        ...
        if (method === null || method === void 0 ? void 0 : method.requires_reference) {
    ...
    const doesPaymentMethodRequireReference = () => {
        ...
        return (method === null || method === void 0 ? void 0 : method.requires_reference) || false;
    };
    return (<Card className="w-full">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            return (<FormItem className="flex flex-col">
                ...
                }} value={((_a = field.value) === null || _a === void 0 ? void 0 : _a.toString()) || ''} disabled={loadingStudents}>
                    <FormControl>
                      <SelectTrigger>
            ...
            {selectedStudent && (<div className="mt-6">
                ...
                {loadingFees ? (<p className="text-center py-4">Loading fees...</p>) : outstandingFees.length === 0 ? (<div className="bg-yellow-50 p-4 rounded-md flex items-center gap-2 text-yellow-800">
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {outstandingFees.map((fee) => (<TableRow key={fee.id} className={`cursor-pointer ${(selectedFee === null || selectedFee === void 0 ? void 0 : selectedFee.id) === fee.id ? 'bg-primary/10' : ''}`} onClick={() => handleFeeSelection(fee)}>
                            <TableCell>
                              <Checkbox checked={(selectedFee === null || selectedFee === void 0 ? void 0 : selectedFee.id) === fee.id} onCheckedChange={() => handleFeeSelection(fee)}/>
                            </TableCell>
            ...
            {selectedFee && (<>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  {/* Payment Amount */}
                  <FormField control={form.control} name="amount_paid" render={({ field }) => (<FormItem>
                  ...
                  {/* Payment Date */}
                  <FormField control={form.control} name="payment_date" render={({ field }) => (<FormItem>
                  ...
                  {/* Installment Number */}
                  <FormField control={form.control} name="installment_number" render={({ field }) => (<FormItem>
                  ...
                  {/* Payment Method */}
                  <FormField control={form.control} name="payment_method" render={({ field }) => (<FormItem>
                    ...
                    handlePaymentMethodChange(value);
                            ...
                            {paymentMethods.length > 0 ? (paymentMethods.map(method => (<div key={method.id} className="flex items-center space-x-2">
                  ...
                  {doesPaymentMethodRequireReference() && (<FormField control={form.control} name="reference_number" render={({ field }) => (<FormItem>
                  ...
                  {/* Remarks */}
                  <FormField control={form.control} name="remarks" render={({ field }) => (<FormItem className="col-span-full">
                  ...
                  {/* Generate Receipt */}
                  <FormField control={form.control} name="generate_receipt" render={({ field }) => (<FormItem className="flex flex-row items-center space-x-3 space-y-0 col-span-full">
                        <FormControl>
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={processingPayment || !selectedFee} className="flex items-center gap-2">
                {processingPayment ?
            'Processing...' :
            <>
      ...
      <CardFooter className="bg-muted/50 flex justify-between">
        ...
        {/* Payment Summary */}
        {selectedFee && (<div className="text-right">
