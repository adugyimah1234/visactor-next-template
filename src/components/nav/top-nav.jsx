...
import { Bell, Menu, User, ChevronDown, LogOut, Settings, HelpCircle, KeyRound } from "lucide-react";
...
export default function TopNav({ title }) {
    ...
    return (<Container className="flex h-16 items-center justify-between border-b border-border">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button className="p-2 rounded-md hover:bg-accent relative disabled:opacity-50" onClick={handleNotificationClick} disabled={isLoadingNotifications}>
            {isLoadingNotifications ? (<Loader size="sm" showText={false}/>) : (<>
                <Bell size={20}/>
                {notifications.filter(n => !n.read).length > 0 && (<span className="absolute top-1 right-1 bg-red-500 rounded-full w-2 h-2"></span>)}
              </>)}
          </button>

          {/* Notifications dropdown */}
          {isNotificationsOpen && (<div className="absolute right-0 mt-2 w-80 rounded-md border border-border bg-card shadow-lg z-10">
              {/* ... notifications list ... */}
            </div>)}
        </div>
        ...
        {/* User Menu */}
        <div className="relative">
          <button className="flex items-center gap-2 p-1 pl-2 rounded-md hover:bg-accent disabled:opacity-50" onClick={() => setIsUserMenuOpen(!isUserMenuOpen)}>
            <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden">
              {isLoadingProfile ? (<Loader size="sm" showText={false}/>) : profileImage ? (<Image src={profileImage} alt="User avatar" width={32} height={32} className="h-full w-full object-cover"/>) : (<User size={16} className="text-primary"/>)}
            </div>
          ...
          {/* User dropdown menu */}
          {isUserMenuOpen && user && (<div className="absolute right-0 mt-2 w-56 rounded-md border border-border bg-card shadow-lg z-10">
              ...
              <ul>
                <li>
                  <button onClick={handleProfileClick} className="w-full text-left px-3 py-2 hover:bg-accent flex items-center gap-2" disabled={isLoadingProfile}>
                    {isLoadingProfile ? (<Loader size="sm" showText={false}/>) : (<User size={16}/>)}
                    <span>Profile</span>
                  </button>
                </li>
                <li>
                  <button className="w-full text-left px-3 py-2 hover:bg-accent flex items-center gap-2">
                    {isLoadingSettings ? (<Loader size="sm" showText={false}/>) : (<Settings size={16}/>)}
                    <span>Settings</span>
                  </button>
                </li>
                <li>
                ...
                <li className="border-t border-border">
                  <button onClick={handleLogout} className="w-full text-left px-3 py-2 hover:bg-accent flex items-center gap-2 text-red-500 disabled:opacity-50">
                    {isLoggingOut ? (<Loader size="sm" showText={false} className="text-red-500"/>) : (<LogOut size={16}/>)}
                    <span>Log out</span>
                  </button>
                </li>
              </ul>
      ...
      {/* Mobile menu */}
      {isMobileMenuOpen && (<div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm md:hidden">
          <div className="fixed inset-y-0 left-0 w-3/4 max-w-xs bg-card p-6 shadow-lg">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-xl font-semibold">{title}</h2>
              <button className="p-2 rounded-md hover:bg-accent" onClick={() => setIsMobileMenuOpen(false)}>
                ✕
              </button>
            </div>
