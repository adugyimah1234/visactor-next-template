...
import { DollarSign, BookOpen, Home, Shirt, FileText, PenTool, Activity } from 'lucide-react';
...
const PaymentBreakdownComponent = () => {
    ...
    useEffect(() => {
        const fetchPaymentData = async () => {
            try {
                ...
                const categoryIdToName = Object.fromEntries(categoryList.map(cat => [cat.id, cat.name]));
                const receipts = (receiptsRaw || []).filter(r => r.student_id !== null && !isNaN(Number(r.amount)));
                const receiptTypeStats = Object.keys(receiptTypeConfig).map(type => {
                    ...
                    return {
                        type: receiptTypeConfig[type].name,
                        amount: totalAmount,
                        count: typeReceipts.length,
                        color: receiptTypeConfig[type].color,
                        icon: receiptTypeConfig[type].icon
                    };
                ...
                const receiptTypeCategories = Object.keys(receiptTypeConfig).map(type => {
                    ...
                    return {
                        category: receiptTypeConfig[type].name,
    ...
    if (error) {
        return (<div className="bg-red-50 border border-red-200 rounded-lg p-6 flex items-center gap-3">
