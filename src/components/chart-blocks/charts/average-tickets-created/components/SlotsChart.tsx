...
const SchoolSummaryCard = ({ 
  ...
  color,
  classes 
}: SchoolSlotData & { color: string }) => {
const getStatusInfo = (rate: number, overbooked: boolean) => {
  if (overbooked) return { status: "overbooked", icon: AlertTriangle, color: "text-red-700 bg-red-100" };
  if (rate >= 90) return { status: "critical", icon: AlertTriangle, color: "text-red-600 bg-red-50" };
  if (rate >= 75) return { status: "high", icon: TrendingUp, color: "text-orange-600 bg-orange-50" };
  if (rate >= 50) return { status: "moderate", icon: TrendingUp, color: "text-blue-600 bg-blue-50" };
...
const statusInfo = getStatusInfo(utilization_rate, overbooked);
  ...
  return (
    <div className=" dark::bg-white rounded-lg border p-4 hover:shadow-md transition-shadow">
...
export default function SlotsChart() {
  ...
  const fetchSlotData = async () => {
    try {
      ...
      const schoolClassMap = new Map<number, ClassData[]>();
      classes.forEach(cls => {
      ...
      const classStudentCount = new Map<number, number>();
      students.forEach((student: any) => {
        if (student.class_id && student.admission_status === 'admitted') {
          classStudentCount.set(
            student.class_id, 
            (classStudentCount.get(student.class_id) || 0) + 1
          );
        }
      });
...
const total_slots = schoolClasses.reduce((sum, cls) => sum + cls.slots, 0);
  ...
  // Get school colors
  const getSchoolColor = (index: number) => schoolColors[index % schoolColors.length];
  ...
  className="px-4 py-2 bg-slate-600 rounded-md border border-gray-300 text-white text-sm hover:bg-slate-600 transition"
>
  Refresh Data
</button>
              ...
              {chartType === 'line' ? (
                <LineChart data={timeSeriesData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <ReferenceLine 
                    ...
                    stroke="#ef4444" 
                    strokeDasharray="3 3" 
                    label={{ value: "Capacity Alert", position: "right" }}
                  />
                  {schoolSlotData.map((school, index) => (
                    <Line
                      ...
                      stroke={getSchoolColor(index)}
                      strokeWidth={2}
                      dot={{ fill: getSchoolColor(index), strokeWidth: 2, r: 4 }}
                      activeDot={{ r: 6, stroke: getSchoolColor(index), strokeWidth: 2 }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">
                        {schoolSlotData.find(s => s.utilization_rate >= 80)?.school_name || 'None'} 
