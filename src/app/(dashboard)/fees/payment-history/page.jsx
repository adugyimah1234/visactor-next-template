...
import { Calendar as CalendarIcon, Download, Printer, Mail, RefreshCw } from "lucide-react";
...
export default function PaymentHistoryPage() {
    ...
    const formatCurrency = (amt) => new Intl.NumberFormat("en-GH", { style: "currency", currency: "GHS" }).format(amt);
    const getReceiptTypeBadge = (type) => {
        ...
        return map[type] || { variant: "default", label: type };
    };
    function renderStudentName(receipt) {
        var _a, _b;
        ...
        return (receipt.student_name ||
            `${(_a = receipt.registration_first_name) !== null && _a !== void 0 ? _a : ""} ${(_b = receipt.registration_last_name) !== null && _b !== void 0 ? _b : ""}`.trim() ||
            "Unknown");
    }
    ...
    return (<div className="space-y-6">
      ...
      {/* School Tabs */}
      <Tabs value={activeSchool !== null && activeSchool !== void 0 ? activeSchool : ((_c = (_b = (_a = schools[0]) === null || _a === void 0 ? void 0 : _a.id) === null || _b === void 0 ? void 0 : _b.toString()) !== null && _c !== void 0 ? _c : "none")} onValueChange={(val) => setActiveSchool(val)}>
        <TabsList>
          {schools.map((s) => (<TabsTrigger key={s.id} value={s.id.toString()}>
              {s.name}
            </TabsTrigger>))}
        </TabsList>
        ...
        {schools.map((school) => {
            ...
            const selectedStudents = students.filter((s) => `${s.school_id}` === schoolId &&
            ...
            const selectedApplicants = applicants.filter((a) => `${a.school_id}` === schoolId &&
            ...
            const filteredReceipts = receipts.filter((receipt) => {
                ...
                if (!isStudentInClass && !isApplicantInClass)
                    return false;
                ...
                const student = students.find((s) => Number(s.id) === Number(receipt.student_id)) ||
            ...
            const allTypes = Array.from(new Set(filteredReceipts.flatMap((r) => { var _a, _b; return (_b = (_a = r.receipt_items) === null || _a === void 0 ? void 0 : _a.map((i) => i.receipt_type)) !== null && _b !== void 0 ? _b : []; }))).filter(Boolean);
            const selectedReceiptTab = activeReceiptTab[schoolId] || "all";
            const receiptTabData = (selectedReceiptTab === "all"
                ? filteredReceipts
                : filteredReceipts.filter((r) => r.receipt_type === selectedReceiptTab)).filter((r) => {
                var _a, _b;
                // Filter out registration-only receipts
                const types = (_b = (_a = r.receipt_items) === null || _a === void 0 ? void 0 : _a.map((i) => i.receipt_type)) !== null && _b !== void 0 ? _b : [];
                return !types.every((t) => t === "registration");
            });
            return (<TabsContent key={school.id} value={schoolId} className="space-y-6">
              {/* Class Tabs */}
              <Tabs value={selectedClassTab} onValueChange={(val) => setActiveClassTab((prev) => (Object.assign(Object.assign({}, prev), { [schoolId]: val })))}>
                <TabsList className="flex flex-wrap gap-2 mb-4">
                  <TabsTrigger value="all">All Classes</TabsTrigger>
                  {schoolClasses.map((c) => (<TabsTrigger key={c.id} value={c.id.toString()}>
                      {c.name}
                    </TabsTrigger>))}
                </TabsList>
                ...
                <TabsContent value={selectedClassTab}>
                  {/* Receipt Type Tabs */}
                  <Tabs value={selectedReceiptTab} onValueChange={(val) => setActiveReceiptTab((prev) => (Object.assign(Object.assign({}, prev), { [schoolId]: val })))}>
                    <TabsList className="flex flex-wrap gap-2 mb-4">
                      <TabsTrigger value="all">All Receipts</TabsTrigger>
                      {allTypes.map((t) => (<TabsTrigger key={t} value={t}>
  ...
  {filteredReceipts.map((receipt, index) => {
                    ...
                    const studentForCalc = students.find(s => Number(s.id) === Number(receipt.student_id)) ||
                        applicants.find(a => Number(a.id) === Number(receipt.registration_id));
                    // Gather all receipts for this student/applicant up to and including this receipt
                    const allStudentReceiptsUpToCurrent = receipts.filter(r => (Number(r.student_id) === Number(receipt.student_id) && r.student_id !== null) ||
                        (Number(r.registration_id) === Number(receipt.registration_id) && r.registration_id !== null)).filter(r => Number(r.id) <= Number(receipt.id));
                    ...
                    allStudentReceiptsUpToCurrent.forEach(r => {
                        var _a;
                        ((_a = r.receipt_items) !== null && _a !== void 0 ? _a : [{ receipt_type: r.receipt_type }]).forEach(item => {
                    ...
                    if (studentForCalc && categories.length && classes.length) {
                        totalAmount = calculateStudentTotalDue(studentForCalc, categories, classes, []);
                        const amountLeft = calculateStudentTotalDue(studentForCalc, categories, classes, Array.from(paidTypes));
                        balance = formatCurrency(amountLeft);
                    }
                    const amountPaid = (_a = receipt.amount) !== null && _a !== void 0 ? _a : 0;
        ...
        <TableCell className="space-x-1">
          {((_b = receipt.receipt_items) !== null && _b !== void 0 ? _b : []).map((i, idx) => (<Badge key={`${i.receipt_type}-${idx}`} variant={getReceiptTypeBadge(i.receipt_type).variant}>
