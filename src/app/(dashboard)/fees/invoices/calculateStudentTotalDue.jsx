export function calculateStudentTotalDue(student, // Student | RegistrationData
categories, classes, paidTypes = []) {
    var _a, _b;
    if (!student)
        return 0;
    const isApplicant = student.class_applying_for !== undefined;
    ...
    const categoryName = ((_a = categories.find(c => Number(c.id) === Number(categoryId))) === null || _a === void 0 ? void 0 : _a.name) || "";
    // With this (handles both ID and name):
    const classNameRaw = ((_b = classes.find(cls => Number(cls.id) === Number(classId) ||
        cls.name.trim().toLowerCase() === String(classId).trim().toLowerCase())) === null || _b === void 0 ? void 0 : _b.name) || "";
