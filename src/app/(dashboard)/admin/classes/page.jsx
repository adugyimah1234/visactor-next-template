...
export default function AdminSchoolsPage() {
    ...
    async function selectSchool(school) {
        ...
        try {
            ...
            setClasses(schoolClasses.map((cls) => {
                var _a;
                return (Object.assign(Object.assign({}, cls), { slots: (_a = cls.slots) !== null && _a !== void 0 ? _a : 0 }));
            }));
        }
        catch (_a) {
    ...
    function onSchoolChange(e) {
        ...
        if (!selectedSchool)
            return;
        setSelectedSchool((prev) => (prev ? Object.assign(Object.assign({}, prev), { [name]: value }) : prev));
    }
    // Handle class changes
    function onClassChange(id, field, value) {
        setClasses((prev) => prev.map((cls) => {
            var _a;
            return ((_a = cls.id) !== null && _a !== void 0 ? _a : cls.temp_id) === id
                ? Object.assign(Object.assign({}, cls), { [field]: field === "slots" ? Number(value) : value }) : cls;
        }));
    }
    // Add new empty class
    function addClass() {
        ...
        try {
            ...
            const classRequests = classes.map((cls) => {
                ...
                if (cls.id >= 1000000000) {
                    ...
                    return classService.update(Object.assign({ id: cls.id }, payload));
                }
            });
    ...
    return (<div className="max-w-7xl mx-auto p-6 flex gap-6">
      ...
      <Card className="w-1/3 max-h-[80vh] overflow-auto">
        ...
        <CardContent className="space-y-2">
          ...
          <ul>
            {schools.map((schoolItem) => (<li key={schoolItem.id} className={`cursor-pointer p-3 rounded-md ${(selectedSchool === null || selectedSchool === void 0 ? void 0 : selectedSchool.id) === schoolItem.id
                ? "bg-primary text-primary-foreground font-semibold"
                : "hover:bg-muted"} flex justify-between items-center`} onClick={() => selectSchool(schoolItem)}>
                <span>{schoolItem.name}</span>
                <Button size="sm" variant="destructive" onClick={(e) => {
                e.stopPropagation();
                if (window.confirm(`Are you sure you want to delete school "${schoolItem.name}"?`)) {
      ...
      <div className="flex-1 space-y-6 overflow-auto max-h-[80vh]">
        {!selectedSchool ? (<Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input id="name" name="name" value={selectedSchool.name} onChange={onSchoolChange} placeholder="Example High School" required/>
                </div>
                <div>
                  <Label htmlFor="address">Address</Label>
                  <Textarea id="address" name="address" value={selectedSchool.address} onChange={onSchoolChange} placeholder="123 Main St, City" rows={2} required/>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="phone_number">Phone Number</Label>
                    <Input id="phone_number" name="phone_number" value={selectedSchool.phone_number} onChange={onSchoolChange} placeholder="+1234567890" type="tel" required/>
                  </div>
                  <div>
                    <Label htmlFor="email">Email</Label>
                    <Input id="email" name="email" value={selectedSchool.email} onChange={onSchoolChange} placeholder="email@example.com" type="email" required/>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {classes.length === 0 && (<p className="text-sm text-muted-foreground">
                    No classes added yet.
                  </p>)}

                {classes.map((cls) => {
                var _a, _b, _c, _d;
                return (<div key={(_a = cls.id) !== null && _a !== void 0 ? _a : cls.temp_id} className="grid grid-cols-5 gap-4 items-center border-b border-border py-2">
    <div>
      <Label htmlFor={`class-name-${(_b = cls.id) !== null && _b !== void 0 ? _b : cls.temp_id}`}>
        Class Name
      </Label>
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Input value={cls.name} placeholder="Type or select class" onChange={(e) => { var _a; return onClassChange((_a = cls.id) !== null && _a !== void 0 ? _a : cls.temp_id, "name", e.target.value); }}/>
        </PopoverTrigger>
        <PopoverContent className="p-0">
          <div className="flex flex-col">
            {CLASS_OPTIONS.map((option) => (<button key={option} className={cn("text-left w-full px-4 py-2 hover:bg-muted")} onClick={() => { var _a; return onClassChange((_a = cls.id) !== null && _a !== void 0 ? _a : cls.temp_id, "name", option); }}>
                {option}
              </button>))}
          </div>
        </PopoverContent>
    ...
    <div>
      <Label htmlFor={`class-slots-${(_c = cls.id) !== null && _c !== void 0 ? _c : cls.temp_id}`}>
        Slots Available
      </Label>
      <Input id={`class-slots-${(_d = cls.id) !== null && _d !== void 0 ? _d : cls.temp_id}`} type="number" min={0} value={cls.slots} onChange={(e) => { var _a; return onClassChange((_a = cls.id) !== null && _a !== void 0 ? _a : cls.temp_id, "slots", e.target.value); }} required/>
    </div>
      ...
      <Button variant="destructive" size="sm" onClick={() => {
        ...
        Delete
            ...
            <div className="flex justify-end gap-2">
              <Button onClick={onSave} disabled={saving} className="w-40" type="button">
