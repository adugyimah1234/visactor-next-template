...
export default function ModuleAccess() {
    ...
    const fetchUsers = async () => {
        try {
            ...
            setUsers(data.map((user) => (Object.assign(Object.assign({}, user), { name: user.name || '', role: user.role || 'user', createdAt: user.createdAt || new Date().toISOString(), updatedAt: user.updatedAt || new Date().toISOString() }))));
        }
        catch (error) {
     ...
     */
    const fetchUserModuleAccess = async (userId) => {
        try {
    ...
    const handleModuleToggle = async (moduleId, checked) => {
        ...
        try {
            ...
            // Optimistic update
            setModuleAccess(prev => (Object.assign(Object.assign({}, prev), { [moduleId]: checked })));
            // If toggling a parent module on, also toggle all children
            const parentModule = modules.find(m => m.id === moduleId);
            if ((parentModule === null || parentModule === void 0 ? void 0 : parentModule.children) && checked) {
                const updatedAccess = Object.assign(Object.assign({}, moduleAccess), { [moduleId]: checked });
            ...
            if (checked) {
                for (const module of modules) {
                    if ((_a = module.children) === null || _a === void 0 ? void 0 : _a.some(child => child.id === moduleId)) {
                        setModuleAccess(prev => (Object.assign(Object.assign({}, prev), { [module.id]: true })));
                        break;
                    }
                }
            }
     ...
     */
    const isUpdatePending = (moduleId) => {
    ...
    return (<Card className="shadow-md">
      <CardHeader>
        <div className="flex justify-between items-center">
          ...
          <Button variant="outline" size="sm" onClick={refreshAll} disabled={isLoadingUsers || isLoadingModules || isLoadingAccess}>
            <RefreshCw className={`h-4 w-4 mr-2 ${(isLoadingUsers || isLoadingModules || isLoadingAccess) ? 'animate-spin' : ''}`}/>
            Refresh
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        ...
        {userError && (<Alert variant="destructive" className="mb-4">
            <AlertCircle className="h-4 w-4"/>
            <AlertTitle>Error</AlertTitle>
            <AlertDescription>{userError}</AlertDescription>
            <Button variant="outline" size="sm" onClick={fetchUsers} className="mt-2" disabled={isLoadingUsers}>
        ...
        {moduleError && (<Alert variant="destructive" className="mb-4">
            <AlertCircle className="h-4 w-4"/>
            <AlertTitle>Error</AlertTitle>
            <AlertDescription>{moduleError}</AlertDescription>
            <Button variant="outline" size="sm" onClick={fetchModules} className="mt-2" disabled={isLoadingModules}>
        ...
        <div className="mb-6">
          ...
          {isLoadingUsers ? (<div className="space-y-2">
              <Skeleton className="h-10 w-[300px]"/>
            </div>) : users.length === 0 ? (<Alert variant="default" className="mb-4">
              <Info className="h-4 w-4"/>
              <AlertTitle>No Users Found</AlertTitle>
              <AlertDescription>No users available. Please add users to manage module access.</AlertDescription>
            </Alert>) : (<Select onValueChange={handleUserChange} value={selectedUser}>
        ...
        {/* Selected user info */}
        {selectedUser && selectedUserDetails && (<div className="mb-6 p-4 border rounded-md bg-muted/30">
        ...
        {accessError && selectedUser && (<Alert variant="destructive" className="mb-4">
            <AlertCircle className="h-4 w-4"/>
            <AlertTitle>Error</AlertTitle>
            <AlertDescription>{accessError}</AlertDescription>
            <Button variant="outline" size="sm" onClick={refreshModuleAccess} className="mt-2" disabled={isLoadingAccess}>
        ...
        {/* Module access table */}
        {selectedUser ? (isLoadingAccess || isLoadingModules ? (<div className="space-y-4">
              <Skeleton className="h-8 w-full"/>
              <Skeleton className="h-12 w-full"/>
              <Skeleton className="h-12 w-full"/>
              <Skeleton className="h-12 w-full"/>
              <Skeleton className="h-12 w-full"/>
            </div>) : modules.length === 0 ? (<Alert variant="default" className="mb-4">
              <Info className="h-4 w-4"/>
                ...
                return (<Fragment key={module.id}>
                    <TableRow>
                      ...
                      <TableCell className="text-center">
                        {isUpdatePending(module.id) ? (<Spinner className="h-4 w-4 mx-auto"/>) : (<Checkbox checked={moduleAccess[module.id] || false} onCheckedChange={(checked) => handleModuleToggle(module.id, checked === true)} aria-label={`Toggle access to ${module.name}`}/>)}
                      </TableCell>
                    </TableRow>
                    {(_a = module.children) === null || _a === void 0 ? void 0 : _a.map(subModule => (<TableRow key={subModule.id} className="bg-muted/50">
                        ...
                        <TableCell className="text-center">
                          {isUpdatePending(subModule.id) ? (<Spinner className="h-4 w-4 mx-auto"/>) : (<Checkbox checked={moduleAccess[subModule.id] || false} onCheckedChange={(checked) => handleModuleToggle(subModule.id, checked === true)} disabled={!moduleAccess[module.id]} aria-label={`Toggle access to ${subModule.name}`}/>)}
                        </TableCell>
                      </TableRow>))}
                  </Fragment>);
            })}
              </TableBody>
            </Table>)) : (<Alert variant="default" className="mt-6">
            <Info className="h-4 w-4"/>
